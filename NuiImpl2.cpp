...
void CSkeletalViewerApp::Nui_DrawSkeleton( bool bBlank, NUI_SKELETON_DATA * pSkel, HWND hWnd, int WhichSkeletonColor )
{
	...
	float t[NUI_SKELETON_POSITION_COUNT*3];
	//float t2[NUI_SKELETON_POSITION_COUNT*3];
	//std::vector<double> headPos;
	//headPos.push_back(pSkel->SkeletonPositions[3].x);
	//headPos.push_back(pSkel->SkeletonPositions[3].y);
	//headPos.push_back(pSkel->SkeletonPositions[3].z);
    for (i = 0; i < NUI_SKELETON_POSITION_COUNT; i++)
    {
		...
		/*const int size=1024;
		char msg[size];
		_snprintf(msg, size, "%f,%f,%f\n",  pSkel->SkeletonPositions[i].x,  pSkel->SkeletonPositions[i].y,  pSkel->SkeletonPositions[i].z);
		sender.send(msg, strlen(msg));*/
		double x = pSkel->SkeletonPositions[i].x;//-headPos[0];
		double y = pSkel->SkeletonPositions[i].y;//-headPos[1];
		double z = pSkel->SkeletonPositions[i].z;//-headPos[2];
		//double rel_x = (headPos.at(0)-pSkel->SkeletonPositions[i].x);
		//double rel_y = (headPos.at(1)-pSkel->SkeletonPositions[i].y);
		//double rel_z = (headPos.at(2)-pSkel->SkeletonPositions[i].z);
		//double norm = std::sqrt((rel_x*rel_x)+(rel_y*rel_y)+(rel_z*rel_z));
		//if(norm > 0)
		//{
		//	rel_x /= norm;
		//	rel_y /= norm;
	...
	//sender2.send(t2,sizeof(t2));
	
	/*float *t = new float(vector.size());

	for (int i=0; i<vector.size(); i++)
		t[i] = vector[i];

	sender.send(t, vector.size());
	*/
