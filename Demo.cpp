...
void senderThread(void* x = NULL) {
	while (1) {
		float data[4];
		if (remote.IsBalanceBoard()) {
			data[0] = float(remote.BalanceBoard.Kg.TopL);
			data[1] = float(remote.BalanceBoard.Kg.TopR);
			data[2] = float(remote.BalanceBoard.Kg.BottomL);
			data[3] = float(remote.BalanceBoard.Kg.BottomR);
			sender->send(data,sizeof(data));
			//cout << "Sending " << sizeof(emotivData) << " bytes..." << endl;
		}
		Sleep(8);
	}
}
...
int main(int argc, char **argv)
	...
	for (int i = 1; i < argc; i++) {
		if (skipNextArg) {
			...
			skipNextArg = true;
			if (i + 1 > argc) {
				throw std::exception("Invalid number of connections specified.");
			}
			numConnections = atoi(argv[i + 1]);
			if (numConnections <= 0 || numConnections >= 8)
				throw std::exception("Invalid number of connections specified.");
		} else if (_strcmpi(argv[i], "-p") == 0) {
			skipNextArg = true;
			if (i + 1 > argc) {
				throw std::exception("Invalid port number specified.");
			}
			portNum = atoi(argv[i + 1]);
			cout << "Asking for port " << portNum << "..." << endl;
			if (portNum < 0) {
				throw std::exception("Invalid port number specified.");
			}
		} else {
			throw std::exception("Invalid argument specified.");
		}
	}

	std::cout << "Waiting for " << numConnections << " connections." << std::endl;

	// LELSender Additions
	sender = new LELSender(numConnections, portNum);
	// Send on a separate thread.
	_beginthread(senderThread,0,0);
		...
		switch(remote.ExtensionType)
			...
			case wiimote_state::BALANCE_BOARD:
				...
				WHITE; _tprintf(_T("TL    "));
					   _tprintf(_T("%6.2f"), float(remote.BalanceBoard.Lb.TopL));
				CYAN ; _tprintf(_T(" kg")); WHITE; ;_tprintf(_T("  TR "));
					   _tprintf(_T("%6.2f"), float(remote.BalanceBoard.Lb.TopR));
				CYAN;  _tprintf(_T(" kg\n")); WHITE;
					   _tprintf(_T("                                    BL    "));
					   _tprintf(_T("%6.2f"),float( remote.BalanceBoard.Lb.BottomL));
				CYAN;  _tprintf(_T(" kg")); WHITE; _tprintf(_T("  BR "));
					   _tprintf(_T("%6.2f"), float(remote.BalanceBoard.Lb.BottomR));
				CYAN;  _tprintf(_T(" kg \n")); WHITE;
					   _tprintf(_T("                                    Total "));
					   _tprintf(_T("%6.2f"), remote.BalanceBoard.Lb.Total);
				CYAN;  _tprintf(_T(" kg"));
				}
				break;
