...
HRESULT CSkeletalViewerApp::Nui_Init( )
{
	...
	//ROSS T - Adding audio flag here!  9/6/2012
...
DWORD WINAPI CSkeletalViewerApp::Nui_ProcessThread( )
{
	...
	//sender.setNumRequestedConnections(4);
	//sender.setNumRequestedConnections(1);
...
void CSkeletalViewerApp::HandleWiiFit(void)
{
	float fourWeights[4] = {0.f, 0.f, 0.f, 0.f};
	if(remote.IsConnected())
	{
		...
		fourWeights[0] = remote.BalanceBoard.Lb.BottomL;
		fourWeights[1] = remote.BalanceBoard.Lb.BottomR;
		fourWeights[2] = remote.BalanceBoard.Lb.TopL;
		fourWeights[3] = remote.BalanceBoard.Lb.TopR;
	...
	if(useFiona)
	{
		...
		//fiona packets is shared between threads here so we might need to mutex
		if(fionaConnected)
		{
			UpdateWiiFitData *pWii = new UpdateWiiFitData(BasePacket::UPDATE_WIIFIT);
			pWii->SetPayload(fourWeights);
			fionaPackets.push_back(pWii);
		}
		LeaveCriticalSection(&cs);
	}
	else
	{
		sender.SetBalanceBoardValues(fourWeights);
	}
}

void CSkeletalViewerApp::HandleEMG(void)
{
	/*EnterCriticalSection(&cs);
	if(fionaConnected)
	{
		...
		pEMG->SetPayload(values);
		fionaPackets.push_back(pEMG);
	}
	LeaveCriticalSection(&cs);*/

	//also handle emg here..
	/*if(comportOpen)
	{
		const unsigned int maxNumSize = 32;
		...
		{
			EnterCriticalSection(&cs);
			//convert the buffer values into integers as Dan described..
			if(fionaConnected)
			{
				UpdateEMGData *pEMG = new UpdateEMGData(BasePacket::UPDATE_EMG);
				pEMG->SetPayload(values);
...
bool CSkeletalViewerApp::Nui_GotColorAlert( )
{
	...
	/*if (processedFrame && playerVideo)
	{
		playerVideo->handleColorData( imageFrame );
	}*/
...
bool CSkeletalViewerApp::Nui_GotDepthAlert( )
{
	...
	//if (processedFrame && playerVideo)
	//	playerVideo->handleDepthData( imageFrame );
	...
	if (m_SendVideoFrames && playerVideo && playerVideo->getData(vd))
	{


	
		bool framesInQueue = false;
		for (auto it = fionaPackets.begin(); it != fionaPackets.end(); ++it)
		{
			if ((*it)->GetCommandType() == BasePacket::UPDATE_KINECT_VIDEO)
			{
				framesInQueue = true;
				break;
			}
		...
		if (!framesInQueue)
		{
			UpdateKinectVideoData* uk = new UpdateKinectVideoData( vd );
			fionaPackets.push_back( uk );
		}
	}
	LeaveCriticalSection(&cs);
	*/
...
{      
    int i;
float t[NUI_SKELETON_POSITION_COUNT*(3+4)];

    NUI_SKELETON_BONE_ORIENTATION boneOrientations[NUI_SKELETON_POSITION_COUNT];
    NuiSkeletonCalculateBoneOrientations(&skel, boneOrientations);
	//boneOrientations[0].absoluteRotation.rotationQuaternion.
    ...
    for (i = 0; i < NUI_SKELETON_POSITION_COUNT; i++)
    {
		...
		//skel.SkeletonPostions has the 3d positions we want.. RDT 2012
		t[i*7+0] = skel.SkeletonPositions[i].x;
		t[i*7+1] = skel.SkeletonPositions[i].y;
		t[i*7+2] = skel.SkeletonPositions[i].z;
		//rotationQuaternion to make the limb segment rotation relative to the proximal joint
		t[i*7+3] = boneOrientations[i].hierarchicalRotation.rotationQuaternion.w;
		t[i*7+4] = boneOrientations[i].hierarchicalRotation.rotationQuaternion.x;
		t[i*7+5] = boneOrientations[i].hierarchicalRotation.rotationQuaternion.y;
		t[i*7+6] = boneOrientations[i].hierarchicalRotation.rotationQuaternion.z;
		//if (i==5)
		//{
		//	printf("%d %d\n",boneOrientations[i].startJoint, boneOrientations[i].endJoint);
		//	t[i*7+3] = boneOrientations[i].startJoint; // 2
		//	t[i*7+4] = boneOrientations[i].endJoint;  // 4 
		//}
   }
	if(!useFiona)
	{
		sender.StoreSkeleton(t);
	}
	else
	{
		EnterCriticalSection(&CSkeletalViewerApp::cs);
		//if(fionaPackets.size() > 0)
		//{
			bool hasKinect = false;
			std::vector<BasePacket*>::iterator iter = fionaPackets.begin();
			while(iter != fionaPackets.end())
			{
				if((*iter)->GetCommandType() == BasePacket::UPDATE_KINECT)
				{
					hasKinect = true;
					break;
				}
			...
			if(!hasKinect)
			{
				UpdateKinectData *pK = new UpdateKinectData(BasePacket::UPDATE_KINECT);
				pK->SetPayload(t);
				fionaPackets.push_back(pK);
			}
		//}
